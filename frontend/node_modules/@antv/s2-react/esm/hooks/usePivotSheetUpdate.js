 ...
 */
export const usePivotSheetUpdate = (partDrillDown) => {
    const prePartDrillDownRef = React.useRef(partDrillDown);
    /** 属性值发生变化时，才更新 callback，触发底表 render */
    const callbackDeps = DRILL_DOWN_ATTR_TO_DIFF.map((attr) => get(partDrillDown, attr));
    return React.useCallback((renderOptions) => {
        const prePartDrillDown = prePartDrillDownRef.current;
        /** 对应属性的新老值有改变，则以 reload 方式 render */
        const shouldReloadData = !!DRILL_DOWN_ATTR_TO_DIFF.find((attr) => !Object.is(get(partDrillDown, attr), get(prePartDrillDown, attr)));

...
import { CellType, KEY_GROUP_CORNER_RESIZE_AREA, ResizeAreaEffect, ResizeDirectionType, S2Event, } from '../common/constant';
...
export class CornerCell extends HeaderCell {
    ...
    drawTreeIcon() {
        ...
        const isAllCollapsed = values(collapseFields).filter(Boolean).length === rootRowNodes.length ||
    ...
    drawResizeArea() {
        ...
        const resizeArea = getOrCreateResizeAreaGroupById(this.spreadsheet, KEY_GROUP_CORNER_RESIZE_AREA);
        if (!resizeArea) {
            return;
        }
    ...
    getContentPosition({ contentWidth = this.getActualTextWidth(), } = {}) {
        ...
        const { size = 0 } = this.getStyle().icon;
        const iconY = getVerticalIconPosition(size, textY, textStyle.fontSize, textStyle.textBaseline);

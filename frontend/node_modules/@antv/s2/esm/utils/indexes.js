 ...
 * @param indexes
 */
export const isXYInRange = (x, y, indexes) => {
    const [xMin, xMax, yMin, yMax] = indexes;
    return x >= xMin && x <= xMax && y >= yMin && y <= yMax;
};
export const allIndexes = (indexes) => {
 ...
 * @param targetIndexes
 */
export const diffIndexes = (sourceIndexes, targetIndexes) => {
    const add = [];
    const remove = [];
    // source 为空
...
export const diffPanelIndexes = (sourceIndexes, targetIndexes) => {
    ...
    Object.keys(targetIndexes || {}).forEach((key) => {
        const { add, remove } = diffIndexes((sourceIndexes === null || sourceIndexes === void 0 ? void 0 : sourceIndexes[key]) || [], targetIndexes[key]);
        allAdd.push(...add);

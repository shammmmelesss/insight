...
class RowColumnClick extends base_event_1.BaseEvent {
    constructor() {
        super(...arguments);
        ...
        this.handleRowColClick = (event) => {
            ...
            if (cell instanceof cell_1.SeriesNumberCell) {
                return;
            }
            ...
            const multiSelectionName = cell.cellType === constant_1.CellType.ROW_CELL
                ? constant_1.InteractionName.ROW_CELL_MULTI_SELECTION
                : constant_1.InteractionName.COL_CELL_MULTI_SELECTION;
            const defaultSelectionName = cell.cellType === constant_1.CellType.ROW_CELL
                ? constant_1.InteractionName.ROW_CELL_CLICK
                : constant_1.InteractionName.COL_CELL_CLICK;
    ...
    showTooltip(event) {
        const { operation, enable: showTooltip } = (0, tooltip_1.getTooltipOptions)(this.spreadsheet, event);
        if (!showTooltip) {
            return;
        }
        const { interaction } = this.spreadsheet;
        const cellInfos = interaction.isSelectedState()
            ? (0, tooltip_1.mergeCellInfo)(interaction.getActiveCells())
            : [];
    ...
    getHeaderTooltipOperator(event, operation) {
        ...
        const isColCell = cell.cellType === constant_1.CellType.COL_CELL;
        // 只有一个叶子节点时, 不显示隐藏按钮
        const isOnlyOneLeafColumn = this.spreadsheet.facet.getColLeafNodes().length === 1;
        const TOOLTIP_OPERATOR_HIDDEN_COLUMNS_MENU = (0, constant_1.getTooltipOperatorHiddenColumnsMenu)();
    ...
    bindTableColExpand() {
        this.spreadsheet.on(constant_1.S2Event.COL_CELL_EXPANDED, (node, hideDirection) => {
    ...
    hideSelectedColumns() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            // 兼容多选
            yield (0, hide_columns_1.hideColumnsByThunkGroup)(this.spreadsheet, selectedColumnFields, true);
        });
    }
    handleExpandIconClick(node, hideDirection) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const { hideColumnNodes = [] } = (0, lodash_1.findLast)(lastHiddenColumnsDetail, ({ displaySiblingNode }) => (0, hide_columns_1.isEqualDisplaySiblingNodeId)(displaySiblingNode, node.id, hideDirection)) || {};
            ...
            const hiddenColumnFields = (0, lodash_1.difference)(lastHideColumnFields, willDisplayColumnFields);
            const hiddenColumnsDetail = lastHiddenColumnsDetail.filter(({ displaySiblingNode }) => !(0, hide_columns_1.isEqualDisplaySiblingNodeId)(displaySiblingNode, node.id, hideDirection));

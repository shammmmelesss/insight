...
exports.BaseBrushSelection = void 0;
...
const interaction_2 = require("../../utils/interaction");
const base_interaction_1 = require("../base-interaction");
class BaseBrushSelection extends base_interaction_1.BaseEvent {
    constructor() {
        super(...arguments);
        this.displayedCells = [];
        this.brushRangeCells = [];
        this.brushSelectionStage = interaction_1.InteractionBrushSelectionStage.UN_DRAGGED;
        ...
        this.autoScrollConfig = (0, lodash_1.cloneDeep)(interaction_1.BRUSH_AUTO_SCROLL_INITIAL_CONFIG);
        ...
        this.validateXIndex = (xIndex) => {
            ...
            const frozenGroupAreas = facet.frozenGroupAreas;
            let min = 0;
            const frozenColRange = frozenGroupAreas[constant_1.FrozenGroupArea.Col].range;
            ...
            const frozenTrailingColRange = frozenGroupAreas[constant_1.FrozenGroupArea.TrailingCol].range;
        ...
        this.adjustNextColIndexWithFrozen = (colIndex, dir) => {
            ...
            if (colCount > 0 &&
                dir === constant_1.ScrollDirection.SCROLL_UP &&
                (0, utils_1.isFrozenCol)(colIndex, colCount)) {
                return panelIndexes[0];
            }
            if (trailingColCount > 0 &&
                dir === constant_1.ScrollDirection.SCROLL_DOWN &&
                (0, utils_1.isFrozenTrailingCol)(colIndex, trailingColCount, colLength)) {
        ...
        this.adjustNextRowIndexWithFrozen = (rowIndex, dir) => {
            ...
            if (rowCount > 0 &&
                dir === constant_1.ScrollDirection.SCROLL_UP &&
                (0, utils_1.isFrozenRow)(rowIndex, cellRange.start, rowCount)) {
                return panelIndexes[2];
            }
            if (trailingRowCount > 0 &&
                dir === constant_1.ScrollDirection.SCROLL_DOWN &&
                (0, utils_1.isFrozenTrailingRow)(rowIndex, cellRange.end, trailingRowCount)) {
                return panelIndexes[3];
            }
            return rowIndex;
        };
        this.getWillScrollRowIndexDiff = (dir) => {
            return dir === constant_1.ScrollDirection.SCROLL_DOWN
                ? interaction_1.ScrollDirectionRowIndexDiff.SCROLL_DOWN
                : interaction_1.ScrollDirectionRowIndexDiff.SCROLL_UP;
        };
        ...
        this.getNextScrollDelta = (config) => {
            ...
            if (config.y.scroll) {
                const dir = config.y.value > 0
                    ? constant_1.ScrollDirection.SCROLL_DOWN
                    : constant_1.ScrollDirection.SCROLL_UP;
                const willScrollToRowIndex = this.getWillScrollToRowIndex(dir);
                if ((0, lodash_1.isNil)(willScrollToRowIndex)) {
                    y = 0;
                }
                else {
                    const scrollOffsetY = (0, interaction_2.getScrollOffsetForRow)(willScrollToRowIndex, dir, this.spreadsheet) -
                        scrollY;
                    const isInvalidScroll = (0, lodash_1.isNil)(scrollOffsetY) || Number.isNaN(scrollOffsetY);
            ...
            if (config.x.scroll) {
                ...
                x = (0, lodash_1.isNil)(nextIndex)
                    ? 0
                    : (0, interaction_2.getScrollOffsetForCol)(nextIndex, dir, this.spreadsheet) - scrollX;
            }
            return {
                x,
                y,
        ...
        this.handleScroll = (0, lodash_1.throttle)((x, y, isRowHeader = false) => {
            if (this.brushSelectionStage === interaction_1.InteractionBrushSelectionStage.UN_DRAGGED) {
                return;
            }
        ...
        this.showPrepareSelectedCells = () => {
            ...
            this.spreadsheet.interaction.changeState({
                cells: (0, lodash_1.map)(this.brushRangeCells, (item) => (0, interaction_2.getCellMeta)(item)),
                stateName: constant_1.InteractionStateName.PREPARE_SELECT,
                /*
        ...
        this.renderPrepareSelected = (point) => {
            ...
            const cell = elements
                ...
                .find(Boolean);
            // 只有行头，列头，单元格可以刷选
            const isBrushCellType = cell instanceof cell_1.DataCell ||
                cell instanceof cell_1.RowCell ||
                cell instanceof cell_1.ColCell;
    ...
    initPrepareSelectMaskShape() {
        ...
        this.prepareSelectMaskShape = foregroundGroup.appendChild(new g_1.Rect({
            style: {
                width: 0,
                height: 0,
                x: 0,
                ...
                zIndex: constant_1.FRONT_GROUND_GROUP_BRUSH_SELECTION_Z_INDEX,
                visibility: 'hidden',
                pointerEvents: 'none',
            },
        }));
    ...
    resetDrag() {
        ...
        this.setBrushSelectionStage(interaction_1.InteractionBrushSelectionStage.UN_DRAGGED);
    }
    ...
    mouseDown(event) {
        ...
        this.setBrushSelectionStage(interaction_1.InteractionBrushSelectionStage.CLICK);
    ...
    bindMouseUp(enableScroll = false) {
        ...
        this.spreadsheet.on(constant_1.S2Event.GLOBAL_MOUSE_UP, (event) => {
            if (this.brushSelectionStage !== interaction_1.InteractionBrushSelectionStage.DRAGGED) {
                this.resetDrag();
                return;
            }
            if (enableScroll) {
            ...
            if (this.isValidBrushSelection()) {
                ...
                const tooltipData = (0, utils_2.getCellsTooltipData)(this.spreadsheet);
                this.spreadsheet.showTooltipWithInfo(event, tooltipData);
            }
            if (this.spreadsheet.interaction.getCurrentStateName() ===
                constant_1.InteractionStateName.PREPARE_SELECT) {
                this.spreadsheet.interaction.reset();
            }
            this.resetDrag();
        });
        // 刷选过程中右键弹出系统菜单时, 应该重置刷选, 防止系统菜单关闭后 mouse up 未相应依然是刷选状态
        this.spreadsheet.on(constant_1.S2Event.GLOBAL_CONTEXT_MENU, () => {
            if (this.brushSelectionStage === interaction_1.InteractionBrushSelectionStage.UN_DRAGGED) {
                return;
            }
            this.spreadsheet.interaction.removeIntercepts([constant_1.InterceptType.HOVER]);
            this.resetDrag();
        });
    }
    ...
    emitBrushSelectionEvent(event, scrollBrushRangeCells, detail) {
        ...
        this.spreadsheet.emit(constant_1.S2Event.GLOBAL_SELECTED, scrollBrushRangeCells, detail);
        // 未刷选到有效单元格, 允许 hover
        if ((0, lodash_1.isEmpty)(scrollBrushRangeCells)) {
            this.spreadsheet.interaction.removeIntercepts([constant_1.InterceptType.HOVER]);
        }
    }
...
exports.BaseBrushSelection = BaseBrushSelection;
